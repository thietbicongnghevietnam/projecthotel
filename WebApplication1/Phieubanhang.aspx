...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <div class="col-sm-4">
                        <h5 class="m-0">F2(Sửa nhanh)  - F4(Xóa nhanh) - F8(Thêm) - F9(Ghi HĐ)</h5>
                    </div>
                    <div class="col-sm-4">
                       <span class="addphieunhap" style="display: inline-flex;
    align-items: center;
    padding: 8px 12px;
    ...
    transition: background-color 0.3s;">                
                    ...
                    <span class="editphieunhap" style="
    display: inline-flex;
    align-items: center;
    padding: 8px 12px;
    background-color: #ffc107; /* màu vàng Bootstrap */
    color: black;              /* màu chữ đậm dễ đọc trên nền vàng */
    border: none;
    border-radius: 4px;
    cursor: pointer;
    font-size: 16px;
...
">                
                        ...
                        <span class="addKhachhang" style="
    display: inline-flex;
    align-items: center;
    padding: 8px 12px;
    background-color: #d1ecf1; /* xanh da trời nhạt */
    color: #0c5460;            /* chữ xanh đậm */
    border: 1px solid #bee5eb; /* viền nhẹ xanh */
    border-radius: 4px;
    cursor: pointer;
    font-size: 16px;
    transition: background-color 0.3s;
">
                        <i class="fa fa-user" style="font-size: 24px; padding-left: 3px;"></i>
    ...
    <table id="dnhaphang" style="padding-bottom: 10px; padding-right: 10px;padding-top: 20px;padding-left: 20px;">
                ...
                <td>
                    ...
                    <td>
                            <div style="display: flex; align-items: center; padding: 5px 10px;">
                                <label for="inhoadon" style="margin: 0; white-space: nowrap;">In hóa đơn</label>
                                <input type="checkbox" id="inHD" name="inHD" style="margin-left: 10px; transform: scale(1.1);">
                            </div>
                        </td>

                        <!-- Cột trống (có thể thêm gì đó sau) -->
                        <td></td>

                        <!-- Cột Sửa HĐ -->
                        <td>
                            <div style="display: flex; align-items: center; padding: 5px 10px;">
                                <b style="color: black; white-space: nowrap;">| Sửa HĐ</b>
                                <input type="checkbox" id="xemlaiHD" name="xemlaiHD" style="margin-left: 10px; transform: scale(1.1);">
                                <input id="suahd_soluongcu" class="form-control input-sm" value="0"
                    ...
                    <td>
                        ...
                        <input type="checkbox" id="xemlaiHD" name="xemlaiHD">                                          
                         <input id="suahd_soluongcu" class="form-control input-sm" value="0" style="width:60px;float:right; margin-left:2px;">
                    </td>--%>
                ...
                <span style="float: left; padding-right: 10px; padding-top: 20px; padding-left: 50px;">
                    <input type="submit" value="Làm Mới" id="lammoiid"  class="btn btn-success float-right">   
                </span>
            ...
            <div class="modal-dialog" >
                <div class="modal-content">
                    <div class="modal-header">
                        <div class="row">
                            <div>
                                <h4 class="modal-title" id="headerTag1" style="float: left">Thêm mới khách hàng</h4>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal Header -->
      <div class="modal-header">
        <h4 class="modal-title">Danh sách hàng hóa cần tìm</h4>
    ...
    <script>
     ...
     <script type="text/javascript">
         ...
         function handleMahang(mahang, ktdvt, soluong_dvt, tongtienhang, dvtid) {
             ...
             if (gettr_rs == 'Remove') {
                 ...
                 $('#tbphieunhap tr').each(function () {
                     ...
                     //new                                
                     if (ktdvt == "==DVT==") {
                         soluongmoi = parseInt($("#soluongnhaphang").val());
                     }
                     else {
                         soluongmoi = parseInt(soluong_dvt) * parseInt($("#soluongnhaphang").val());
                     }
         ...
         $('#tb_damuchanghoa tr').keydown(function (e) {
             var keyCode = e.keyCode || e.which;
             if (keyCode === 38 || keyCode === 40) { // 38 là phím mũi tên lên, 40 là phím mũi tên xuống
                 //console.log('Đã di chuyển dòng');
                 // Thực hiện các thao tác cần thiết khi di chuyển dòng
             }
         });
         ...
         document.addEventListener('keydown', function (event) {
             if (event.key === 'F9') {                
                 if ($('#chietkhauid').val() == 0 && $('#thanhtoanid').val() == 0 && $('#conlaiid').val() ==0) {
                     alert("Kiem tra lai hoa don!");
                 } else {
                     ghilaihoadon();
                 }                 
             }
        ...
        $('#lammoiid').click(function () {
            $.ajax({
                         type: "POST",
                         contentType: "application/json; charset=utf-8",
                         url: "Phieubanhang.aspx/Reset_edit_temp",
                         //data: JSON.stringify(data2),
                         dataType: "json",
                         success: function (data2) {
                             //alert(data.d);  
         ...
         //ham kiem tra chuoi co dau cach hay khong?
         function containsWhiteSpace(str) {
             return str.includes(' ');
         }

         $("#dvtid").change(function () {
             var dvtto = $(this).val(); // Lấy giá trị đã chọn của combobox
             var mahang = $('#MainContent_phieunhaphang').val();

             //lay ra don gia cua cau thanh don vi tinh
             var data = { mahang: mahang, dvtto: dvtto };
             $.ajax({
                 type: "POST",
                 contentType: "application/json; charset=utf-8",
                 url: "Phieubanhang.aspx/dongiacauthanhdvt",
                 data: JSON.stringify(data),
                 dataType: "json",
                 success: function (data) {
                     //const objdata = $.parseJSON(data.d);
             ...
             //luon vao truong hop ma hang
             //var checktenhang = 0;
             //if (containsWhiteSpace(mahang) == true)
             //{
             //    alert('truong hop la ten hang');
             //    checktenhang = 1;
             //}
             //else
             //{
             //     alert('truong hop la mahang hang');
             //    checktenhang = 0;
             //}            
         });
